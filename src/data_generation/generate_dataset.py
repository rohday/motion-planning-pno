...
def generate_random_map(size, rng, min_free=0.55, max_free=0.75,
                        min_obstacle_px=20):
    """Generate a clean binary occupancy map with blob-like obstacles.

    The topology is enforced in three steps:
      1) smooth the base noise so connected blobs form naturally,
      2) remove tiny obstacle islands, and
      3) keep only the largest connected free-space region.

    Boundary walls are added last so they are never removed by morphology.
    """
    for _ in range(200):
